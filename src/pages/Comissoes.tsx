  ...
  return (
    <>
      <div className="mb-5">
        ...
        <p className="mt-1 text-[13px] text-[#8D95A3]">Acompanhamento de comissões por pedido e por indústria</p>
      </div>
      ...
      <div className="mb-5 grid grid-cols-1 gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2" title="Comissão por mês">
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={trend}>
              <CartesianGrid stroke="#212833" vertical={false} />
              <XAxis dataKey="m" stroke="#8D95A3" fontSize={12} tickLine={false} axisLine={false} />
              <YAxis stroke="#8D95A3" fontSize={12} tickLine={false} axisLine={false} width={40} />
              <Tooltip
                contentStyle={{ background: '#171C24', border: '1px solid #2A313D', borderRadius: 6, fontSize: 12 }}
        ...
        <Card title="Por indústria">
          <div className="space-y-3">
            {byIndustry.map(({ ind, sales, commission }) => (
              <div key={ind.id}>
                <div className="mb-1 flex items-center justify-between text-[12.5px]">
                  ...
                  <span className="mono text-[#C7CCD6]">{currency(commission)}</span>
                </div>
            ...
            {byIndustry.length === 0 && <p className="text-[12.5px] text-[#8D95A3]">Sem pedidos registrados ainda.</p>}
          </div>
        </Card>
      </div>

      <Card className="!p-0 overflow-hidden">
        <div className="hidden grid-cols-[0.8fr_1.4fr_1fr_0.9fr_0.9fr_0.9fr] gap-3 border-b border-[#2A313D] px-5 py-3 text-[11.5px] font-medium text-[#8D95A3] md:grid">
          <span>Pedido</span>
          <span>Cliente</span>
          <span>Data</span>
          <span>Total</span>
          ...
          {sorted.map((o) => {
            ...
            return (
              <div key={o.id} className="grid grid-cols-2 gap-2 px-5 py-3 text-[13px] md:grid-cols-[0.8fr_1.4fr_1fr_0.9fr_0.9fr_0.9fr] md:items-center md:gap-3">
                ...
                <span className="mono text-[#C7CCD6]">{currency(orderTotal(o))}</span>
                <span className="mono text-[#3FA9A0]">{currency(orderCommission(o))} <span className="text-[#8D95A3]">({o.comissaoPercentual}%)</span></span>
                <span className="text-[11.5px] text-[#8D95A3]">{ORDER_STATUS_LABEL[o.status]}</span>
              </div>
            )
          })}
        </div>
