    ...
    <div className="flex h-[calc(100dvh-96px)] flex-col sm:h-[calc(100dvh-104px)]">
      <div className="mb-4 flex items-center gap-2.5">
        ...
        <div>
          ...
          <p className="text-[12px] text-[#8D95A3]">Pergunte sobre sua carteira ou peça para agendar um follow-up</p>
        </div>
      </div>

      <div className="flex-1 space-y-3 overflow-y-auto rounded-md border border-[#2A313D] bg-[#1A1F27] p-4">
        {aiMessages.map((m) => (
          <div key={m.id} className={`flex ${m.autor === 'usuario' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] whitespace-pre-line rounded-[10px] px-3.5 py-2.5 text-[13px] ${m.autor === 'usuario' ? 'bg-[#E2963C]/15 text-[#F2F0EA]' : 'bg-[#212833] text-[#F2F0EA]'}`}>
              {m.texto}
              <div className="mono mt-1.5 text-[10px] text-[#8D95A3]">{formatTime(m.hora)}</div>
            </div>
          </div>
        ))}
        <div ref={endRef} />
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        {SUGGESTIONS.map((s) => (
          <button key={s} onClick={() => submit(s)} className="rounded-full border border-[#2A313D] bg-[#171C24] px-2.5 py-1 text-[11.5px] text-[#8D95A3] hover:text-[#C7CCD6]">
            {s}
          </button>
        ))}
      </div>
