  ...
  return (
    <>
      <div className="mb-5">
        ...
        <p className="mt-1 text-[13px] text-[#8F8676]">Rotas em andamento, planejadas e concluídas</p>
      </div>
          ...
          <div className="space-y-2">
            {active.map((r) => (
              <button
                ...
                className="flex w-full items-center justify-between rounded-[6px] border border-[#3FA9A0]/30 bg-[#3FA9A0]/10 px-3 py-2.5 text-left"
              >
                <span className="flex items-center gap-2 text-[13px] text-[#2B2620]"><Play size={13} className="text-[#3FA9A0]" /> {r.nome}</span>
          ...
          <div className="space-y-2">
            {planned.map((r) => (
              <button
                key={r.id}
                onClick={() => navigate(`/rotas?id=${r.id}`)}
                className="flex w-full items-center justify-between rounded-[6px] border border-[#E4DCC8] bg-[#F3EEE3] px-3 py-2.5 text-left hover:bg-[#ECE3D2]"
              >
                <div>
      ...
      {concluded.length > 0 && (
        <>
          ...
          <Card className="!p-0 overflow-hidden">
            {sorted.length === 0 ? (
              <div className="px-5 py-10 text-center text-[13px] text-[#8F8676]">Nenhuma rota concluída nesse período.</div>
            ) : (
                ...
                {sorted.map((r) => {
                  ...
                  return (
                    <button
                      key={r.id}
                      onClick={() => navigate(`/rotas?id=${r.id}`)}
                      className="flex w-full items-center justify-between gap-3 px-5 py-3.5 text-left hover:bg-[#F3EEE3]"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-[13px] font-medium text-[#2B2620]">{r.nome}</div>
                          <div className="mt-0.5 flex items-center gap-3 text-[11.5px] text-[#8F8676]">
