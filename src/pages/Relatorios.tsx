  ...
  return (
    <>
      <div className="mb-5">
        ...
        <p className="mt-1 text-[13px] text-[#6B7F93]">Visão consolidada de vendas, comissões e carteira</p>
      </div>
      ...
      <div className="mb-5 grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card title="Vendas por indústria">
          {salesByIndustry.length === 0 ? (
            <p className="py-8 text-center text-[12.5px] text-[#6B7F93]">Sem pedidos registrados.</p>
          ) : (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={salesByIndustry} layout="vertical" margin={{ left: 8 }}>
                <CartesianGrid stroke="#E1EDFB" horizontal={false} />
                <XAxis type="number" stroke="#6B7F93" fontSize={11} tickLine={false} axisLine={false} />
                <YAxis dataKey="nome" type="category" stroke="#6B7F93" fontSize={11} tickLine={false} axisLine={false} width={130} />
        ...
        <Card title="Carteira por classificação">
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              ...
              <Tooltip contentStyle={{ background: '#EAF3FC', border: '1px solid #CFE0F5', borderRadius: 6, fontSize: 12 }} formatter={(v, n) => [String(v), STATUS_LABEL[n as ClientStatus]]} />
            </PieChart>
          </ResponsiveContainer>
          <div className="flex flex-wrap justify-center gap-3">
            {clientsByStatus.map((r) => (
              <span key={r.status} className="flex items-center gap-1.5 text-[11.5px] text-[#6B7F93]">
      ...
      <Card title="Top clientes por faturamento">
        <div className="space-y-3">
          {topClients.map(({ client, valor }) => (
            <div key={client.id}>
              <div className="mb-1 flex items-center justify-between text-[12.5px]">
                <span className="text-[#33495E]">{client.nomeFantasia ?? client.razaoSocial}</span>
          ...
          {topClients.length === 0 && <p className="py-4 text-center text-[12.5px] text-[#6B7F93]">Sem dados de faturamento ainda.</p>}
        </div>
      </Card>
    </>
  )
