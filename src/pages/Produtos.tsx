    ...
    <>
      <div className="mb-5 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-[13px] text-[#6B7F93]">{products.length} itens no catálogo</p>
        </div>
        <Button onClick={openNew}><Plus size={15} /> Novo produto</Button>
      </div>

      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="flex items-center gap-2 rounded-[6px] border border-[#CFE0F5] bg-[#EAF3FC] px-3 py-2 text-[13px] text-[#6B7F93] sm:max-w-[300px] sm:flex-1">
          <Search size={15} />
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar produto ou SKU…" className="w-full bg-transparent text-[#0F2A44] outline-none placeholder:text-[#6B7F93]" />
        </div>
        <div className="flex flex-wrap gap-1.5">
          <button onClick={() => setIndustryFilter('todas')} className={`rounded-full border px-2.5 py-1 text-[11.5px] ${industryFilter === 'todas' ? 'border-[#3B82F6]/50 bg-[#3B82F6]/15 text-[#3B82F6]' : 'border-[#CFE0F5] text-[#6B7F93]'}`}>Todas</button>
          {industries.map((ind) => (
            <button key={ind.id} onClick={() => setIndustryFilter(ind.id)} className={`rounded-full border px-2.5 py-1 text-[11.5px] ${industryFilter === ind.id ? 'text-[#3B82F6]' : 'border-[#CFE0F5] text-[#6B7F93]'}`} style={industryFilter === ind.id ? { borderColor: `${ind.cor}55`, background: `${ind.cor}1A`, color: ind.cor } : {}}>
              {ind.nome}
            </button>
          ))}
        </div>
      ...
      <Card className="!p-0 overflow-hidden">
        <div className="hidden grid-cols-[2fr_1fr_1fr_0.8fr_0.8fr_60px] gap-3 border-b border-[#CFE0F5] px-5 py-3 text-[11.5px] font-medium text-[#6B7F93] md:grid">
          <span>Produto</span>
          <span>Indústria</span>
          <span>Categoria</span>
            ...
            return (
              <div key={p.id} className="grid grid-cols-2 gap-2 px-5 py-3.5 text-[13px] md:grid-cols-[2fr_1fr_1fr_0.8fr_0.8fr_60px] md:items-center md:gap-3">
                <div className="col-span-2 flex items-center gap-3 md:col-span-1">
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[6px] bg-[#E1EDFB] text-[#6B7F93]"><Package size={15} /></div>
                  <div className="min-w-0">
                    <div className="truncate font-medium text-[#0F2A44]">{p.nome}</div>
                    <div className="mono truncate text-[11.5px] text-[#6B7F93]">{p.sku}</div>
                  </div>
                </div>
                <span className="truncate text-[11.5px]" style={{ color: ind?.cor }}>{ind?.nome}</span>
                <span className="truncate text-[#33495E]">{p.categoria}</span>
                <span className="mono text-[#33495E]">{currency(p.precoTabela)}/{p.unidade}</span>
                <span className="mono text-[#16A34A]">{p.comissaoPercentual ?? ind?.comissaoPadrao ?? 0}%</span>
                <div className="flex gap-1 justify-end md:justify-start">
                  <button onClick={() => openEdit(p)} className="rounded-[6px] p-1.5 text-[#6B7F93] hover:bg-[#EAF3FC] hover:text-[#0F2A44]"><Pencil size={13} /></button>
                  <button onClick={() => confirm('Remover produto?') && deleteProduct(p.id)} className="rounded-[6px] p-1.5 text-[#6B7F93] hover:bg-[#EF4444]/10 hover:text-[#EF4444]"><Trash2 size={13} /></button>
                </div>
              </div>
            )
          })}
          {filtered.length === 0 && <div className="px-5 py-10 text-center text-[13px] text-[#6B7F93]">Nenhum produto encontrado.</div>}
        </div>
      </Card>
